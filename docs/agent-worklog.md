...
- pass: app-not-scaffold
  ...
  tier: 2
  tier_reason: >
    Changes what a generated app declares about itself — licence, description,
    README — and adds two flags to the create surface. No kernel, schema, store or
    controls code is touched. Not tier 0: an app generated before this pass asserts
    a licence its author never chose, so the change alters a legal claim rather than
    prose. Not tier 4: no public API changes shape.
  run:
    - node --test packages/cli/src/create.test.mjs
    - npm run verify:final
  skip:
    - >
      Nothing. Both the repository path and the packed tarball were generated from
      and executed, because the licence and README changes only appear in a
      generated app.
```
...
### app-not-scaffold — three files that belong to the product, not the tool

- **Result:** A generated app no longer describes itself as appcraft's output. Its
  README opens with the app's name and the commands its author will run; its
  `package.json` records `UNLICENSED` and carries no description; no `LICENSE` is
  written unless the app declares one via `--license`, with `--author` (or git
  `user.name`) supplying the copyright line.
- **Rules applied:** `preflight-attested` and `worklog-decision-trail` — recorded
  before the gate this pass changes is trusted. `evidence-over-assertion` — the
  licence and README changes appear only in a generated app, so both the repository
  path and the packed tarball were generated from and run.
  `verification-tier-preclassified` — tier 2 declared above before editing.
- **Rejected alternatives:**
  - *Keeping MIT and filling in the copyright holder.* It reads like a fix and is the
    same defect: the author still never chose MIT. A licence is a grant of rights, and
    a scaffolder is not entitled to make one on the user's behalf. `UNLICENSED` is
    npm's value for "no rights granted" and is the only honest default.
  - *Writing licence text for any SPDX id.* Reproducing a licence from memory risks
    shipping something subtly not the licence it claims to be. Only MIT is written
    out; any other id is recorded in the manifest and the CLI says plainly that the
    text is the user's to add.
  - *Moving the appcraft material out of the app README entirely.* An app whose gate
    drives a browser and whose contract lives in `AGENTS.md` needs to say so
    somewhere. It is one section at the end rather than the opening sentence.
  - *Changing `starter/package.json`.* Its MIT and its description are correct for
    what it is — a workspace member of this repository, and the framework's fixture.
    The conflation is fixed in `createAppManifest`, where the "no longer a workspace
    member" transform already lives.
  - *Changing `.gitignore`.* Reviewed and left alone: `node_modules`, build output,
    test output, env files and editor cruft are what the *app* produces. Nothing in it
    is specific to this repository, so there was nothing to separate.
- **Evidence:**
  - `node --test packages/cli/src/create.test.mjs` — 29 pass, 0 fail. Four are new:
    default carries no licence, `--license MIT` writes the text with a real holder,
    `Apache-2.0` is recorded but not invented, and the README's own sections say
    nothing about appcraft.
  - `npm run verify:final` — exit 0, all seven checks OK, 25 unit, 15 browser, 4 perf.
  - Packed tarball: `npm pack` then `create --license MIT --author "Nick Sullivan"` →
    `license: MIT`, `description: undefined`, `Copyright (c) 2026 Nick Sullivan`.
    Then `npm install` and `npm test` in that app → exit 0.
  - Default generation → `license: UNLICENSED`, no `LICENSE` file, README opening with
    the app's name and a comment inviting the author to describe it.
- **Risks:**
  - `--license` records any string. An SPDX id that does not exist is accepted and
    written to the manifest; npm will complain, appcraft will not.
  - The README test exempts `APPCRAFT_CHROMIUM`, which is a branded name in an app's
    own instructions. It is there because it is how you run this app's tests on a
    machine that already has a browser — but it means the assertion cannot be a plain
    "the word never appears".
  - Apps generated by 0.1.0 still carry the inherited MIT and the scaffold
    description. Nothing migrates them, and nothing warns their authors.
  - `UNLICENSED` is a change of default. Anyone who relied on generated apps being
    MIT — and would not have thought to ask — gets a different manifest from the same
    command.
