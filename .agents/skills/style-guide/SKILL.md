...
1. **The picker** — `tools/style-guide/index.html`. Open it in a browser (or serve it
   ...
   either back.
2. **In conversation** — the same eight properties, asked directly. Faster for a user
...
| Body typeface | the base theme's (default) · a family | `typography.body` |
| Heading typeface | same as body (default) · a family | `typography.heading` |
...
2. Offer the picker. If the user prefers to answer directly, ask the eight in one
