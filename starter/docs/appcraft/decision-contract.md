...
Rule ids and levels for an appcraft application. The browser acceptance matrix cites
these ids, and `e2e/appcraft-acceptance.spec.ts` fails if it cites one that is not
here — so coverage and rules cannot drift apart.
...
| **default** | Normal choice | Use unless the product proves a reason not to |
...
If a rule is checkable *without* product knowledge it is an **invariant**; if
evaluating it *requires* product knowledge it is a **heuristic**.
...
| `facade-owns-state` | invariant | All state reads and writes pass through the appcraft facade. Zod, Jotai and Immer must not appear in product code. |
...
| `projection-graph-acyclic` | invariant | The projection reference graph contains no cycles. |
| `panel-discriminant-persists` | invariant | Reopening the app restores the last active tab or tool. Derived state still rebuilds. |
...
| `layout-archetypes-only` | invariant | Surfaces compose from curated archetypes (master-detail, tabbed-section, canvas, inspector). |
| `theme-tokens-not-literals` | invariant | Product code reads Astryx theme tokens. It does not hard-code colour, font-family or font-size values. |
| `figma-structure-source-of-truth` | invariant | When a Figma URL is supplied, read structure via MCP. Never implement from a screenshot or by eye. |
...
| `transport-not-a-projection` | default | Playhead and transport evaluation is a derived sampler across branches, not a projection. |
...
| `custom-control-justified` | escape-hatch | Authoring a novel primitive instead of composing Astryx requires written justification and stronger coverage. |
